...
# Exit if not first day of month
if (day != '01'):
    sys.exit()
...
apiData = Flow.getResponse(f"{requestURI}/surveys/227030237", token)
...
def getAll(url, token=token, page=1):
    ...
    if data:
        ...
        if nextPageUrl is not None:
            print(f"\rLAST PAGE: {str(page)}", end="")
            getAll(nextPageUrl, token, page+1)
        return "done"
    else:
        print("↻ TOKEN EXPIRED, GENERATING NEW")
        getAll(url, Flow.getAccessToken(), page)
...
for form in forms:
    ...
    for index, questionGroup in enumerate(questionGroups):
        ...
        print(f"\n↹ DOWNLOAD AND TRANSFORM | {checkTime(time.time())}: {questionGroup['name']}")
        getAll(formURI)
        ...
        for qst in metadata:
            ...
            try:
                ...
                if qType == 'GEO':
                    output[f"{qName}_lat"] = output[qName].apply(lambda x: x[0] if x is not None else x)
                    output[f"{qName}_long"] = output[qName].apply(lambda x: x[1] if x is not None else x)
...
for dpId in registrationDataPoints:
    print(f"Collect Email Data for datapoint - {str(dpId)}")
    email_data = {}

    # PLOT LOCATION
    # To get the old datapoint
    plant = [x for x in allResponses['01. Plot Registration']['Plant Registration'] if x['dataPointId'] in dpId][1]
    ...
    plot_map_img = f"https://www.mapquestapi.com/staticmap/v4/getmap?size=640,480&type=map&pois=mcenter,{plot_latlong}"
    plot_map_img = f"{plot_map_img}|&zoom=12&center={plot_latlong}&imagetype=JPEG&traffic=flow&scalebar=false&key=lYrP4vF3Uk5zgTiGGuEzQGwGIVDGuy24"
    plot_link = f"http://maps.google.com/maps?t=k&q=loc:{plot_latlong}"
    ...
    plant_submissionDate = datetime.strptime(plant_submissionDate, '%Y-%m-%d').strftime('%B, %d %Y')
    plot_submission_summary =  f"{plot_submission_summary} {plant_submissionDate}"
    plant_type = plant['Type of Plantation']
    plot_submission_summary = f"{plot_submission_summary}  with {plant_type} in {plot_location}"
    ...
    for item in variables:
        if len(filter_plant_gc) > 0:
            ...
            for item2 in tmp_arr:
                if item2[item] == 'Dead':
                    result.append(f"<span style='color:red;font-weight:bold'>{str(item2['Plant ID'])} {item2[item]}</span>")
                else:
                    result.append(str(item2['Plant ID']) + ' ' + item2[item])
            result = ", ".join(result)
            result = f"<b>{result}</b>"
            conditions.append(f"- {item}: {result}")
        else:
