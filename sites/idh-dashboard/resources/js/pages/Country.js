...
import { Row, Col, Card, Form, Button, Jumbotron, Nav } from "react-bootstrap";
...
class Country extends Component {
    ...
    generateResources(country, list) {
        ...
        file = file.replace(" ", "_");
        const reports = [{ type: "raw", text: "Analyzed Farmer Data", href: ".xlsx" }];
            ...
            <Col md={4} key={"report-" + i}>
                <Card>
                    ...
                    <Card.Footer>
                        ...
                        <hr />
                        {this.state.download ? (
                            <a href={"/files/" + file + x.href} target="_blank" className={aClass}>
                                Download
                            </a>
                        ) : (
        ...
        return (
            <Fragment>
                <Jumbotron className="has-navigation">
                        ...
                        <Col md={12} className="page-title text-center">
                            <div className="sub-content">
                                {list.childrens.map((x, i) => (
                                    <a key={i} className={x.id === this.state.active ? "active" : ""} onClick={(e) => this.getData(x.id)}>
                                        {x.company}
                                    </a>
                                ))}
                            </div>
                    ...
                    <Row>
                        <Nav activeKey={tab} className="align-self-center nav-jumbotron" onSelect={this.changeTab}>
                            <Nav.Link active={"overview" === tab} href={"#country/" + country + "/overview"}>
                                Overview
                            </Nav.Link>
                            <Nav.Link active={"hh_profile" === tab} href={"#country/" + country + "/hh-profile"}>
                                HH Profile
                            </Nav.Link>
                            <Nav.Link active={"farmer_profile" === tab} href={"#country/" + country + "/farmer-profile"}>
                                Farmer Profile
                            </Nav.Link>
                            <Nav.Link active={"farm_practices" === tab} href={"#country/" + country + "/farm-practices"}>
                                Farm Practices
                            </Nav.Link>
                            <Nav.Link active={"resources" === tab} href={"#country/" + country + "/resources"}>
