...
class PageThumbs extends Component {
    ...
    spinnerImage(index) {
        return <BeatLoader css="margin-top:90px" size={20} color={"#03ad8c"} />;
    }
    thumbnails(index, data) {
        return <Card.Img id={`pf_img_${index}`} variant="top" src={`images/${this.props.lists}/${data.galleries[0]}`} onLoad={this.handleImageLoaded.bind(this)} />;
    }
                ...
                <Col className={"container-thumbnails" + show} xs={12} sm={6} md={4} key={index}>
                    <Card>
                        ...
                        <div className="thumbnails">
                            ...
                            {this.thumbnails(index, data)}
                        </div>
                        <Card.Body> {this.description(data.description)} </Card.Body>
                        <Card.Footer className="thumbnails-footer">
                            <div className="text-center">
                                <Button variant="primary" className="btn-card" value={data.id} onClick={this.changePage}>
