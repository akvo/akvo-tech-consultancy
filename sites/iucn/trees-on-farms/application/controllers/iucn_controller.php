...
class Iucn_controller extends CI_Controller {
    ...
    public function get_graph_data(){
        ...
        if (!empty($this->input->post())){
            ...
            $query = "SELECT identifier, submitter, collection_date, q4800002, q20080001, q22140004, q10170002 FROM tof_28030003";
            ...
            if (!empty($response_array['rows'])){
                ...
                foreach ($response_array['rows'] as $row){
                    if (!in_array($row['q10170002'], $countries) && !empty($row['q10170002'])){
                        array_push($countries, $row['q10170002']);
                    }
                }
                foreach ($countries as $country){
                    $country_output = array();
                    $country_output['name'] = $country;
                    $country_output['color'] = $country_colours[$country];
                    $country_output['data'] = array();
                    foreach ($response_array['rows'] as $row){
                        if ($row['q10170002'] == $country && in_array($row['q22140004'], array_keys($levels))){
                ...
                foreach ($fields as $field) {
                  if ($field == "q9760001" || $field == "q18170004") {
                    ...
                    if (in_array($field, $gauges)) {
                      ...
                      foreach ($response_array['rows'] as $row) {
                          if ($field == "q22670002") {
                              foreach ($q22670002_options as $option) {
                                  array_push(${$field."_options_array"}, $option);
                              }
                          } else {
