...
const passvar = survey_form.includes(USING_PASSWORDS)
  ? "_password"
  : "_default_password";
const urlParams = qs.parse(document.location.search, {
...
class Submit extends Component {
  ...
  /* CUSTOM */
  pushApi(submitted) {
    let instanceApi = SAVE_FEATURES.find(
      (x) => x.instance === this.props.value.instanceName
    );
    ...
    if (saveData) {
      ...
      saveData = {
        ...
        display_name: localStorage.getItem("_dataPointName")
          ? localStorage.getItem("_dataPointName")
          : "Untitled",
        form_instance_id: saveData.instanceName,
        form_instance_url: formInstanceUrl,
        submitted: submitted,
        uuid: localStorage.getItem("_dataPointId"),
        updated_at: new Date(),
      };
    }
    if (instanceApi) {
    ...
    if (saveData && instanceApi) {
      axios
        .post("https://" + instanceApi, saveData, {
    ...
    db.files
      ...
      .then((result) => {
        ...
        if (localStorage.getItem("_cache") !== null) {
          axios
            .put(
              API_URL + "form-instance/" + localStorage.getItem("_cache"),
              postData,
              { headers: { "Content-Type": "application/json" } }
            )
            .then((res) => {
          ...
          axios
            .post(API_URL + "form-instance", postData, {
              headers: { "Content-Type": "application/json" },
            })
            .then((res) => {
  ...
  sendData(content) {
    ...
    const uppy = Uppy({ debug: true }).use(AwsS3, {
    ...
    axios
      .post(API_URL + "upload-data", content, {
        headers: { "Content-Type": "application/json" },
      })
      .then((res) => {
        const zip = new JSZip();
                  ...
                  if (result.failed.length > 0) {
                    ...
                    axios
                      ...
                      .then((res) => {
                        ...
                        /* END CUSTOM */
                        PopupSuccess(
                          "New datapoint is sent! clearing form..."
                        ).then((res) => {
                          this.setState({ _showSpinner: false });
                          db.delete();
  ...
  submitForm(e) {
    ...
    const showCustomConfirmation =
      this.state._skipMandatories && !this.props.value.captcha;
  ...
  componentDidMount() {
    ...
    let instance = SAVE_FEATURES.find(
      (x) => x.instance === this.props.value.instanceName
    );
    ...
    if (instance) {
      ...
      localStorage.setItem(
        passvar,
        survey_form.includes(USING_PASSWORDS) ? "" : password
      );
      instanceApi = instance.api;
    }
    if (instanceApi && urlParams.user_id) {
      instance = urlParams.demo
        ? urlParams.demo === "1"
          ? instance.demoApi
          : instanceApi
        : instanceApi;
      axios
        .get("https://" + instance + "/flow-submitter/" + urlParams.user_id)
        .then((res) => {
  ...
  render() {
    const submitIsActive =
      this.state._skipMandatories || this.props.value.submit;
    const captchaIsActive =
      !this.state._skipMandatories && this.props.value.captcha;
    return (
      <Fragment>
        <ReCAPTCHA
        ...
        <div className="submit-block">
          <div style={{ display: submitIsActive ? "block" : "none" }}>
            <label className="form-password-label" htmlFor={"submit-username"}>
              Submitter
            </label>
            <input
            ...
            <label
              className={
                this.state._skipPassword ? "hidden" : "form-password-label"
              }
          ...
          <button
            ...
            className={
              "btn btn-block btn-" + (submitIsActive ? "primary" : "secondary")
            }
